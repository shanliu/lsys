...
import { appSubNotifyGetConfig, appSubNotifySetConfig, AppSubNotifySetConfigParamSchema, type AppSubNotifySetConfigParamType } from "@shared/apis/user/app"
...
export function ListNotifyDrawer({ appId, open, onOpenChange }: ListNotifyDrawerProps) {
    ...
    const configQuery = useQuery({
        queryKey: ['app-sub-notify-config', appId],
        queryFn: ({ signal }) => appSubNotifyGetConfig({ app_id: Number(appId) }, { signal }),
        enabled: open && !!appId
    })

    const configData = getQueryResponseData<{ call_url?: string }>(configQuery.data, {})
    const callbackUrl = configData?.call_url || ""



    const form = useForm<AppSubNotifySetConfigParamType>({
        resolver: zodResolver(AppSubNotifySetConfigParamSchema),
        values: {
            app_id: Number(appId),
            url: callbackUrl
        }
    ...
    const updateMutation = useMutation({
        mutationFn: (data: AppSubNotifySetConfigParamType) => appSubNotifySetConfig(data),
        onSuccess: (result) => {
            if (result.status) {
    ...
    const onSubmit = (data: AppSubNotifySetConfigParamType) => {
    ...
    return (
        <Drawer open={open} onOpenChange={onOpenChange}>
            <DrawerContent>
                <DrawerHeader>
                    <DrawerTitle>子应用通知配置</DrawerTitle>
                    <DrawerDescription>
                        配置子应用的回调通知地址
                    </DrawerDescription>
                </DrawerHeader>
