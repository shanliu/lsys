...
import {
    MailConfigFormSchema,
    type MailConfigFormType,
} from "./config-schema";

interface MailConfigDrawerProps {
    appId: string | number;
...
export function MailConfigDrawer({
    appId,
    dictData,
    open,
    onOpenChange,
}: MailConfigDrawerProps) {
    const toast = useToast();
    const queryClient = useQueryClient();

    const form = useForm<MailConfigFormType>({
        resolver: zodResolver(MailConfigFormSchema),
        defaultValues: {
            priority: 0,
            config_type: 0,
    ...
    const mutation = useMutation({
        mutationFn: (data: MailConfigFormType) =>
            userSenderMailerConfigAdd({ app_id: Number(appId), ...data }),
        onSuccess: () => {
    ...
    const onSubmit = (data: MailConfigFormType) => {
    ...
    return (
        <Drawer open={open} onOpenChange={onOpenChange}>
            <DrawerContent>
                <DrawerHeader>
                    <DrawerTitle>新增邮件发送配置</DrawerTitle>
                    <DrawerDescription>
                        填写邮件发送配置信息
                    </DrawerDescription>
                </DrawerHeader>
                                ...
                                <FormItem>
                                    ...
                                    >
                                        ...
                                        <SelectContent className="max-h-[300px]">
                                            {dictData.mail_config_type?.map((item) => (
