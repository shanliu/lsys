import {
    userSenderMailerTplBodyAdd,
    userSenderMailerTplBodyEdit,
...
import { MailTplFormSchema, MailTplFormType } from "./tpl-schema";

interface MailTplDrawerProps {
    tpl?: UserSenderMailerTplBodyItemType;
    open: boolean;
    onOpenChange: (open: boolean) => void;
}

export function MailTplDrawer({ tpl, open, onOpenChange }: MailTplDrawerProps) {
    const toast = useToast();
    const queryClient = useQueryClient();
    const { appId } = Route.useParams();
    const isEdit = !!tpl;

    const form = useForm<MailTplFormType>({
        resolver: zodResolver(MailTplFormSchema),
        defaultValues: tpl
            ? {
                tpl_id: tpl.tpl_id,
    ...
    const mutation = useMutation({
        mutationFn: (data: MailTplFormType) =>
            isEdit
                ? userSenderMailerTplBodyEdit({ id: tpl.id, tpl_data: data.tpl_data })
                : userSenderMailerTplBodyAdd({ app_id: Number(appId), ...data }),
        onSuccess: () => {
    ...
    const onSubmit = (data: MailTplFormType) => {
