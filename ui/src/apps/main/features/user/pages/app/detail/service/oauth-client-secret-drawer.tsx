import { appOAuthClientSecretAdd, appOAuthClientSecretChange, appOAuthClientSecretDel, appSecretView, type AppOAuthClientSecretAddParamType, type AppOAuthClientSecretChangeParamType, type AppOAuthClientSecretDelParamType } from "@shared/apis/user/app"
...
import { OAuthSecretItem } from "./oauth-client-schema"
import { SecretFormData, SecretFormSchema } from "../setting-schema"

interface OAuthClientSecretDrawerProps {
    appId: string
    open: boolean
    onOpenChange: (open: boolean) => void
}

export function OAuthClientSecretDrawer({ appId, open, onOpenChange }: OAuthClientSecretDrawerProps) {
    ...
    const { data: secretQueryData, isLoading, isError, error } = useQuery({
        queryKey: ['oauth-client-secrets', appId],
        queryFn: async () => {
    ...
    const secretData = secretQueryData?.response?.oauth_secret ?? []

    const addMutation = useMutation({
        mutationFn: (data: AppOAuthClientSecretAddParamType) => appOAuthClientSecretAdd(data),
        onSuccess: (result) => {
            if (result.status) {
                toast.success(result.message)
                queryClient.invalidateQueries({ queryKey: ['oauth-client-secrets', appId] })
            } else {
    ...
    const changeMutation = useMutation({
        mutationFn: (data: AppOAuthClientSecretChangeParamType) => appOAuthClientSecretChange(data),
        onSuccess: async (result, variables) => {
            if (result.status) {
                toast.success(result.message)
                await queryClient.invalidateQueries({ queryKey: ['oauth-client-secrets', appId] })
                setEditingId(null)
            } else {
                toast.error(formatServerError(result))
            }
    ...
    const deleteMutation = useMutation({
        mutationFn: (data: AppOAuthClientSecretDelParamType) => appOAuthClientSecretDel(data),
    ...
    const handleEdit = (secret: OAuthSecretItem) => {
        setEditingId(secret.secret_data)
    }

    const handleSaveEdit = (secret: OAuthSecretItem, newData: SecretFormData) => {
    ...
    const handleDelete = (secret: OAuthSecretItem) => {
    ...
    const isPendingForSecret = (secret: OAuthSecretItem) => {
        return deleteMutation.isPending && deleteMutation.variables?.old_secret === secret.secret_data
    }

    const isEditingSecret = (secret: OAuthSecretItem) => {
        return editingId === secret.secret_data
    }

    const isSavingSecret = (secret: OAuthSecretItem) => {
    ...
    return (
        <Drawer open={open} onOpenChange={onOpenChange}>
            <DrawerContent>
                <DrawerHeader>
                    <DrawerTitle>OAuth密钥管理</DrawerTitle>
                    <DrawerDescription>
                        管理OAuth客户端密钥，用于OAuth 2.0认证
                    </DrawerDescription>
                </DrawerHeader>
...
interface OAuthSecretCardProps {
