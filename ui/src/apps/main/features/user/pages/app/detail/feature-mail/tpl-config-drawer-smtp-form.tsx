import { userSenderMailerSmtpConfigAdd, UserSenderMailerSmtpConfigItemType, userSenderMailerSmtpConfigList, UserSenderMailerTplBodyItemType, userSenderMailerTplBodyList } from "@shared/apis/user/sender-mailer";
...
import {
    EmailChannelType,
    TplConfigSmtpFormSchema,
    type TplConfigSmtpFormType,
} from "./tpl-config-schema";

interface TplConfigDrawerSmtpFormProps {
    appId: string | number;
    onClose: () => void;
}

export function TplConfigDrawerSmtpForm({
    appId,
    onClose,
}: TplConfigDrawerSmtpFormProps) {
    ...
    const { data: smtpConfigData, isLoading, error, refetch } = useQuery({
        queryKey: ["user-smtp-config-list"],
        queryFn: async ({ signal }) => {
            const result = await userSenderMailerSmtpConfigList({}, { signal });
            return result;
        },
    });

    const smtpConfigs = getQueryResponseData<UserSenderMailerSmtpConfigItemType[]>(smtpConfigData, []);

    // 主题模板列表
    const { data: subjectTplData, isLoading: isLoadingSubjectTpl } = useQuery({
        queryKey: ["user-tpl-body-list", "subject", deferredTplSearchMap.subject],
        queryFn: async ({ signal }) => {
    ...
    const subjectTplOptions = getQueryResponseData<UserSenderMailerTplBodyItemType[]>(subjectTplData, []);

    // 正文模板列表
    const { data: bodyTplData, isLoading: isLoadingBodyTpl } = useQuery({
        queryKey: ["user-tpl-body-list", "body", deferredTplSearchMap.body],
        queryFn: async ({ signal }) => {
            const result = await userSenderMailerTplBodyList({
    ...
    const bodyTplOptions = getQueryResponseData<UserSenderMailerTplBodyItemType[]>(bodyTplData, []);

    const form = useForm<TplConfigSmtpFormType>({
        resolver: zodResolver(TplConfigSmtpFormSchema),
        defaultValues: {
    ...
    const mutation = useMutation({
        mutationFn: (data: TplConfigSmtpFormType) =>
            userSenderMailerSmtpConfigAdd({ app_id: Number(appId), ...data }),
        onSuccess: () => {
    ...
    const onSubmit = (data: TplConfigSmtpFormType) => {
                    ...
                    <SelectContent className="max-h-[300px]">
                        {smtpConfigs.map((config) => (
                            <SelectItem key={config.id} value={String(config.id)}>
                                {config.name} ({config.user})<span className="ml-2 text-sm">{config.email}</span>
                            </SelectItem>
                        ))}
                    </SelectContent>
                </Select>
