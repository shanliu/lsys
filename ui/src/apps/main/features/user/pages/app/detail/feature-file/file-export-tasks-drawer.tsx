...
export function FileExportTasksDrawer({
    ...
    const { data, isLoading } = useQuery({
        ...
        refetchInterval: (query) => {
            ...
            const hasPending = tasks.some((t) => t.task.status === 1 || t.task.status === 2);
    ...
    const formatParamsSummary = (task: UserFileExportTaskType): string => {
        try {
            const p = JSON.parse(task.task.export_params || '{}');
            const parts: string[] = [];
            ...
            return task.task.export_params || '-';
        }
    };

    // 完成的文件任务：跳转到文件列表并按 tag export_{id} 过滤
    const handleViewFile = (task: UserFileExportTaskType) => {
        const tag = `export_${task.task.id}`;
        if (task.task.export_type === EXPORT_TYPE_USER_FILE_LIST) {
            navigate({
            ...
            <DrawerContent>
                ...
                <div className="px-4 pb-6 space-y-3 overflow-y-auto">
                    {isLoading ? (
                        ...
                        tasks.map((task) => {
                            const StatusIcon = EXPORT_STATUS_ICON[task.task.status] ?? Clock;
                            const isProcessing = task.task.status === 1 || task.task.status === 2;
                            const isDone = task.task.status === 3;
                            const typeLabel = EXPORT_TYPE_LABELS[task.task.export_type] ?? task.task.export_type;
                            ...
                            return (
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        task.task.status === 4 && 'border-destructive/30 bg-destructive/5',
                                    )}
                                >
                                    {/* ID + 类型 badge + 状态 + 时间 */}
                                    <div className="flex items-center justify-between gap-2">
                                        <div className="flex items-center gap-2 min-w-0 flex-wrap">
                                            <span className="text-xs text-muted-foreground flex-shrink-0">#{task.task.id}</span>
                                            ...
                                            <Badge
                                                className={cn(
                                                    ...
                                                    exportStatusMapper.getClass(task.task.status),
                                                )}
                                            >
                                                <StatusIcon className={cn('h-3 w-3', isProcessing && 'animate-spin')} />
                                                {exportStatusMapper.getText(task.task.status)}
                                            </Badge>
                                        </div>
                                        <span className="text-xs text-muted-foreground flex-shrink-0">
                                            {task.task.add_time ? formatTime(task.task.add_time, TIME_STYLE.RELATIVE_ELEMENT) : '-'}
                                        </span>
                                    </div>
                                    ...
                                    {/* 失败原因 */}
                                    {task.task.status === 4 && task.task.error_message && (
