...
import { Route } from "@apps/main/routes/_main/user/app/$appId/features-mail/config";
...
export default function AppDetailFeatureMailConfigPage() {
  ...
  // user\app_sender_mailer\config_del.md
  // user\app_sender_mailer\config_list.md
  ...
  return (
    <>
      ...
      >
        <MailConfigContent dictData={dictData} />
      </AppDetailNavContainer>
...
interface MailConfigContentProps {
  dictData: TypedDictData<["user_sender_mailer"]>;
}

export function MailConfigContent({ dictData }: MailConfigContentProps) {
  const { appId } = Route.useParams();
  ...
  const {
    ...
    queryFn: async ({ signal }) => {
      const result = await userSenderMailerConfigList(
        {
          app_id: Number(appId),
        },
        { signal }
  ...
  // 从查询结果中提取数据
  const configs = getQueryResponseData<UserSenderMailerConfigItem[]>(configData, []);
  ...
  // 刷新数据
  const refreshData = () => {
    queryClient.refetchQueries({ queryKey: ["mail-config-list"] });
  };

  // 清除缓存并重新加载数据（双击搜索按钮时）
  const clearCacheAndReload = () => {
    countNumManager.reset();
    queryClient.invalidateQueries({ queryKey: ["mail-config-list"] });
  };

  // 定义表格列配置
  const columns: ColumnDef<UserSenderMailerConfigItem>[] = [
    {
      accessorKey: "id",
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        const typeLabel = dictData.mail_config_type?.getLabel(configType) || configType;
...
// 导出 schema 供路由使用
export { MailConfigFilterParamSchema } from './config-schema';
