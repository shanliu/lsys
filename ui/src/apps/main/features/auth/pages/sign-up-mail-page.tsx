...
export default function SignUpMailPage() {
  ...
  const form = useForm<RegisterEmailParamType>({
    resolver: zodResolver(RegisterEmailParamSchema),
    defaultValues: {
      nikename: '',
      email: '',
      code: '',
  ...
  const sendCodeMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await registerEmailCode({
        email,
  ...
  const registerMutation = useMutation({
    mutationFn: async (data: RegisterEmailParamType) => {
      ...
      <form onSubmit={form.handleSubmit((data) => registerMutation.mutate(data))} className="grid gap-3">
        ...
        <Link to='/sign-up/sms'>
          <Button variant='outline' type='button' className="w-full" disabled={registerMutation.isPending}>
            <CardSim className='h-4 w-4' /> 短信注册
          </Button>
        </Link>
      </form>
    </Form>
