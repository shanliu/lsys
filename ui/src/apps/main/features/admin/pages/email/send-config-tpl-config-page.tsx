...
export function EmailSendConfigTplConfigPage() {
  ...
  const onNavigate = (params: EmailSendConfigTplConfigFilterParamType) => searchGo(params);
  const toast = useToast();
  const queryClient = useQueryClient();
  const isMobile = useIsMobile();
  const [tplConfigDrawerOpen, setTplConfigDrawerOpen] = React.useState(false);
  const [detailDrawerOpen, setDetailDrawerOpen] = React.useState(false);
  const [selectedConfig, setSelectedConfig] = React.useState<SystemSenderMailerTplConfigItemType | null>(null);
  ...
  const {
    ...
    queryFn: async ({ signal }) => {
      const result = await systemSenderMailerTplConfigList(
        {
          tpl: filterParam.tpl,
          page: {
            page: currentPage,
  ...
  const columns: ColumnDef<SystemSenderMailerTplConfigItemType>[] = [
    ...
    {
      accessorKey: "config_data.from_email",
      header: "发件人",
      cell: ({ row }) => (
        <div className="text-sm">{row.original.config_data.from_email}</div>
      ),
    },
    {
      accessorKey: "config_data.subject_tpl_id",
      header: "模板ID",
      cell: ({ row }) => (
        <div className="text-xs font-mono">
          <div className="text-muted-foreground">
            主题: {row.original.config_data.subject_tpl_id}
          </div>
          <div className="text-muted-foreground">
            正文: {row.original.config_data.body_tpl_id}
          </div>
        </div>
      ),
    },
  ...
  return (
    ...
    >
      ...
      <EmailSendConfigTplConfigDrawer
        open={tplConfigDrawerOpen}
        onOpenChange={setTplConfigDrawerOpen}
      />

      <EmailSendConfigTplConfigDetailDrawer
