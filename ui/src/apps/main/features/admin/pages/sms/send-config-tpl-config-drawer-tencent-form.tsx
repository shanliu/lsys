import { tencentSmsConfigList, TencentSmsConfigItemType } from "@shared/apis/admin/sender-sms";
import {
    FormControl,
    FormField,
    FormItem,
...
import { SmsProviderType, TencentSmsConfigFormSchema } from "./send-config-tpl-config-schema";
import { z } from "zod";

type TencentFormType = z.infer<typeof TencentSmsConfigFormSchema>;

interface SmsSendConfigTplConfigDrawerTencentFormProps {
    form: UseFormReturn<TencentFormType>;
}

export function SmsSendConfigTplConfigDrawerTencentForm({
    form,
}: SmsSendConfigTplConfigDrawerTencentFormProps) {
    // 获取腾讯云配置列表
    const { data: configData } = useQuery({
        queryKey: ["tencent-sms-config-list"],
        queryFn: async () => {
            const result = await tencentSmsConfigList({});
            return result;
        },
    });
        ...
        <>
            ...
            />
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            <FormLabel>腾讯云配置 <span className={cn("text-red-500")}>*</span></FormLabel>
                            <Select
                            ...
                            >
                                <FormControl>
                                    <SelectTrigger>
                                        <SelectValue placeholder="请选择腾讯云配置" />
                                    </SelectTrigger>
                                </FormControl>
