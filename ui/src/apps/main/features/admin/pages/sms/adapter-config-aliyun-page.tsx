...
import { SmsAdapterConfigAliSmsDrawer } from './adapter-config-aliyun-drawer'
import { smsAdapterConfigModuleConfig } from '../nav-info'


export function SmsAdapterConfigAliyunSmsPage() {
  const toast = useToast()
  const queryClient = useQueryClient()
  const isMobile = useIsMobile()
  const [configDrawerOpen, setConfigDrawerOpen] = React.useState(false)
  const [editingConfig, setEditingConfig] = React.useState<AliSmsConfigItemType | undefined>()
  ...
  const {
    ...
    queryFn: async ({ signal }) => {
      const result = await aliSmsConfigList({}, { signal })
      return result
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => aliSmsConfigDel({ id }),
    onSuccess: () => {
      toast.success('阿里云短信配置删除成功')
  ...
  const refreshData = () => {
    queryClient.refetchQueries({ queryKey: ['ali-sms-config-list'] })
  }

  const columns: ColumnDef<AliSmsConfigItemType>[] = [
    {
      accessorKey: 'id',
          ...
          <DataTableAction className={cn(isMobile ? 'justify-end' : 'justify-center')}>
            ...
            <DataTableActionItem mobileDisplay="display" desktopDisplay="collapsed">
              <ConfirmDialog
                ...
                description={
                  <div>
                    确定要删除阿里云短信配置 <strong>{config.name}</strong> 吗？
                    <br />
                    删除后无法恢复。
                  </div>
                }
