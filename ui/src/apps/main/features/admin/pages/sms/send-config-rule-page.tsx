...
import {
    systemSenderSmsConfigDel,
    systemSenderSmsConfigList,
    systemSenderSmsMapping,
    SystemSenderSmsConfigItemType,
...
import { SmsSendConfigRuleDrawer } from "./send-config-rule-drawer";

export function SmsSendConfigRulePage() {
    ...
    return (
        <>
            ...
            >
                <SmsSendConfigRuleContent dictData={dictData} />
            </SmsSendConfigNavContainer>

            <SmsSendConfigRuleDrawer
...
interface SmsSendConfigRuleContentProps {
    dictData: any;
}

export function SmsSendConfigRuleContent({ dictData }: SmsSendConfigRuleContentProps) {
    ...
    const {
        ...
        queryKey: ["admin-sms-config-list"],
        queryFn: async ({ signal }) => {
            const result = await systemSenderSmsConfigList({ signal });
            return result;
        },
    ...
    // 从查询结果中提取数据
    const configs = getQueryResponseData<SystemSenderSmsConfigItemType[]>(configData, []);

    // 删除配置
    const deleteMutation = useMutation({
        mutationFn: (id: number) => systemSenderSmsConfigDel({ config_id: id }),
        onSuccess: () => {
    ...
    // 刷新数据
    const refreshData = () => {
        queryClient.refetchQueries({ queryKey: ["admin-sms-config-list"] });
    };

    // 定义表格列配置
    const columns: ColumnDef<SystemSenderSmsConfigItemType>[] = [
        {
            accessorKey: "id",
        ...
        },
        {
            ...
            cell: ({ getValue }) => {
                ...
                const typeLabel = dictData.sms_config_type.find((item: DictItemType) => item.key === configType)?.val || configType;
                        ...
                        <DataTableActionItem mobileDisplay="display" desktopDisplay="display">
                            ...
                            >
                                ...
                                >
                                   ...
                                   {isMobile ? <span className="mr-2">删除</span> : ""}
                                </Button>
                            </ConfirmDialog>
                        </DataTableActionItem>
                    </DataTableAction>
