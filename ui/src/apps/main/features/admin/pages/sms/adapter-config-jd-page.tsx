        ...
        <DataTableAction className={cn(isMobile ? 'justify-end' : 'justify-center')}>
          ...
          <DataTableActionItem mobileDisplay="display" desktopDisplay="collapsed">
            <ConfirmDialog
              ...
              description={
                <div>
                  确定要删除京东云短信配置 <strong>{row.original.name}</strong> 吗？
                  <br />
                  删除后无法恢复。
                </div>
              }
    ...
    >
      <div className="flex-1 overflow-hidden">
        <DataTable
          ...
          columns={columns}
          loading={isLoading}
          error={isError || apiError ? <CenteredError error={error || apiError} variant="content" onReset={() => queryClient.refetchQueries({ queryKey: ['jd-sms-config-list'] })} /> : null}
          scrollSnapDelay={300}
