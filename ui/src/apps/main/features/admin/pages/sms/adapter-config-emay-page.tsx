...
import { SmsAdapterConfigEmaySmsDrawer } from './adapter-config-emay-drawer'

import { EmaySmsConfigItemType } from '@shared/apis/admin/sender-sms'

export function SmsAdapterConfigEmaySmsPage() {
    const toast = useToast()
    const queryClient = useQueryClient()
    const isMobile = useIsMobile()
    const [configDrawerOpen, setConfigDrawerOpen] = React.useState(false)
    const [editingConfig, setEditingConfig] = React.useState<EmaySmsConfigItemType | undefined>()

    const { data: configData, isLoading, isError, error } = useQuery({
        queryKey: ['emay-sms-config-list'],
        queryFn: async ({ signal }) => await emaySmsConfigList({}, { signal }),
    ...
    const columns: ColumnDef<EmaySmsConfigItemType>[] = [
        ...
        },
        {
            ...
            header: '服务地址',
            size: 150,
            cell: ({ getValue }) => <div className="font-mono text-sm truncate max-w-[150px]">{getValue<string>()}</div>,
        },
        {
                ...
                <DataTableAction className={cn(isMobile ? 'justify-end' : 'justify-center')}>
                    ...
                    <DataTableActionItem mobileDisplay="display" desktopDisplay="collapsed">
                        <ConfirmDialog
                            ...
                            description={
                                <div>
                                    确定要删除亿美软通短信配置 <strong>{row.original.name}</strong> 吗？
                                    <br />
                                    删除后无法恢复。
                                </div>
                            }
        ...
        >
            <div className="flex-1 overflow-hidden">
                <DataTable
                    ...
                    columns={columns}
                    loading={isLoading}
                    error={isError || apiError ? <CenteredError error={error || apiError} variant="content" onReset={() => queryClient.refetchQueries({ queryKey: ['emay-sms-config-list'] })} /> : null}
                    scrollSnapDelay={300}
