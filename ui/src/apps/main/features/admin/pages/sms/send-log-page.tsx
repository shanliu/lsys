...
import { EXPORT_TYPE_SYSTEM_SMSER_MESSAGE_LIST } from "@shared/apis/admin/export";
...
export function SendLogPage() {
  ...
  const {
    ...
    errors: dictErrors,
    refetch: refetchDict,
  } = useDictData(["admin_sender_sms", "admin_export"] as const);
...
interface SendLogContentProps {
  dictData: TypedDictData<["admin_sender_sms", "admin_export"]>;
}
...
function SendLogContent({ dictData }: SendLogContentProps) {
  ...
  const exportAction = useAdminExportAction({
    exportType: EXPORT_TYPE_SYSTEM_SMSER_MESSAGE_LIST,
    params: {
      tpl_key: filterParam.tpl_key ?? undefined,
      status: filterParam.status ?? undefined,
      body_id: filterParam.body_id ?? undefined,
      snid: filterParam.snid ?? undefined,
      mobile: filterParam.mobile ?? undefined,
    },
  });

  const filterForm = useFilterBarForm<z.infer<typeof SmsLogFilterFormSchema>>({
    defaultValues: {
      tpl_key: filterParam.tpl_key,
      status: filterParam.status,
      body_id: filterParam.body_id,
      ...
      <div className="container mx-auto p-4  lg:px-6 py-5 max-w-[1600px] flex flex-col min-h-0 space-y-5">
        ...
        <div className="flex-shrink-0 mb-1 sm:mb-4">
          <FilterBar form={filterForm} className="bg-card rounded-lg border shadow-sm relative">
            ...
            {/* 状态过滤 */}
            <div className="flex-1 min-w-[140px] max-w-[200px]">
              <FilterDictSelect name="status" placeholder="选择状态" label="发送状态" disabled={isLoading} dictData={dictData.sms_send_status} allLabel="全部" />
            </div>
            {/* 手机号过滤 */}
