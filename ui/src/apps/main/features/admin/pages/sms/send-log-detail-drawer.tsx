...
export function SendLogDetailDrawer({
  ...
  const {
    ...
    queryKey: ["systemSenderSmsMessageView", message.id],
    queryFn: ({ signal }) =>
      systemSenderSmsMessageView(
  ...
  const bodyStatus = createStatusMapper(
    ...
    (status) => dictData.sms_body_status?.getLabel(String(status)) || "发送中",
  );
    ...
    <Drawer open={open} onOpenChange={onClose}>
      ...
      >
        <DrawerHeader className={cn("space-y-3")}>
          <DrawerTitle>短信详情</DrawerTitle>
          <DrawerDescription>查看短信 {message.snid} 的详细信息</DrawerDescription>
        </DrawerHeader>
        ...
        <div className="mt-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <DetailItem label="消息ID" value={message.id} />
              <DetailItem label="模板键值">
                <Badge variant="outline">{message.tpl_key}</Badge>
              </DetailItem>
              <DetailItem label="手机号" value={message.mobile} />
              <DetailItem label="状态">
                <Badge className={smsStatus.getClass(message.status)}>
                  {smsStatus.getText(message.status)}
                </Badge>
              </DetailItem>
