...
export function ResPage() {
  ...
  const {
    data: staticResData,
    isError: staticResError,
    error: staticResErrorData,
    isLoading: staticResLoading,
  } = useQuery({
  ...
  useEffect(() => {
    if (staticResError && staticResErrorData) {
      toast.error(
        formatServerError(staticResErrorData, "系统静态资源模板数据加载失败"),
      );
    }
  ...
  if (staticResLoading) {
    return (
      <RbacNavContainer className={cn("m-4 md:m-6")} {...rbacModuleConfig}>
        <PageSkeletonTable />
      </RbacNavContainer>
    );
  }
...
function ResListContent({ staticResources }: ResListContentProps) {
  ...
  // 处理分页查询结果
  isSuccess && countNumManager.handleQueryResult(resData);
  ...
  const columns: ColumnDef<ResourceItemType>[] = [
    ...
    },
    {
      ...
      header: "资源数据",
      cell: ({ getValue }) => (
        <code className="text-xs px-1.5 py-0.5">{getValue<string>()}</code>
      ),
    },
    {
      accessorKey: "res_type",
      ...
      cell: ({ row }) => {
        ...
        const isSystemResource = staticResources.some(
          (item) => item.res_type === resType,
        );
        return (
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return <code className="text-xs px-1.5 py-0.5">{count ?? 0} 个</code>;
      },
    },
    {
      accessorKey: "op_count",
      header: () => (
        <div className={cn(isMobile ? "" : "text-right")}>关联操作数</div>
      ),
      size: 100,
      cell: ({ getValue, row }) => {
        const count = getValue<number | null | undefined>();
...

    {
      id: "actions",
      header: () => (
        <div className={cn(isMobile ? "" : "text-center")}>操作</div>
      ),
      size: 160,
      cell: ({ row }) => {
        const res = row.original;
      ...
      actions={
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => setOpListDrawerOpen(true)}
          >
