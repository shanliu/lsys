...
import { smsAddNeteaseConfig, smsDelNeteaseConfig, smsEditNeteaseConfig, smsAddAppNeteaseConfig, smsListNeteaseConfig } from '../../../../../common/rest/sender_setting';
...
function SystemAppSmsSettingNeteasesmsBox(props) {
    const {
        rowData,
        onFinish
    } = props;
...
export default function SystemAppSmsSettingNeteasesmsPage(props) {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (row) => {
                let delAction = () => {
                    return smsDelNeteaseConfig({ id: row.id }).then((data) => {
    ...
    switch (changeBoxState.show) {
        case 1:
            showBox = <SystemAppSmsSettingNeteasesmsBox
                onFinish={(id) => {
...
export function AppSmsTplConfigNeteaseAddBox(props) {
    ...
    useEffect(() => {
        ...
        smsListNeteaseConfig({}).then((data) => {
            if (!data.status) {
                setTplConfigData({
                    ...tplConfigData,
                    loading: false
    ...
    return (<Grid item xs={10} ><Form method="post" onSubmit={(e) => {
        ...
        <FormControl fullWidth sx={{
            ...
            <InputLabel size="small" id="res-select-label">选择网易云短信端口配置</InputLabel>
            <Select
                fullWidth
                size='small'
