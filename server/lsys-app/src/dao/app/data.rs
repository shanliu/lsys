...
#[derive(Default)]
pub struct AppAttrParam<'t> {
    //内部功能检测
    pub check_inner_feature: Option<&'t [AppRequestType]>, //ExterLogin OAuthClient SubApp OAuthServer
    //外部功能检测
    pub check_exter_feature: Option<&'t [&'t str]>,
    //获取子应用数量
...
pub struct AppAttrData<'t> {
    ...
    pub exter_feature: Option<Vec<(&'t str, bool)>>, //外部功能及启用状态
    pub sub_app_count: Option<Vec<(i8, i64)>>, //子APP数量
    pub parent_app: Option<AppModel>, //上一级APP信息
}

impl App {
    async fn attr_app_data<'t>(
        &self,
        out_data: Vec<AppModel>,
        app_attr: Option<&AppAttrParam<'t>>,
    ) -> AppResult<Vec<(AppModel, AppAttrData<'t>)>> {
        ...
        let inn_feature_data = if let Some(tmp) = app_attr.check_inner_feature {
            let keys = tmp
                .iter()
                .flat_map(|e| {
                    if e.feature_key().is_empty() {
                        None
                    } else {
                        Some((e, e.feature_key()))
                    }
                })
                .collect::<Vec<_>>();
            if !out_data.is_empty() && !keys.is_empty() {
                sqlx::query_as::<_,(u64,String)>(&sql_format!(
                    ...
                    keys.iter().map(|e|e.1).collect::<Vec<_>>(),
                    AppFeatureStatus::Enable as i8
                )).fetch_all(&self.db).await?
                .into_iter()
                .flat_map(|e|{
                    keys.iter().find(|t|t.1==e.1.as_str()).map(|t|(e.0,t.0))
                })
                .collect::<Vec<_>>()
            } else {
                vec![]
        ...
        let ext_feature_data = if let Some(tmp) = app_attr.check_exter_feature {
            let keys = tmp
                .iter()
                .flat_map(|e| {
                    if e.trim().is_empty() {
                        ...
                        Some(self.exter_feature_key(e))
                    }
                })
                .collect::<Vec<_>>();
            let rlen = AppRequestType::ExterFeatuer.feature_key().len() + 1;
            if !out_data.is_empty() && !keys.is_empty() {
                sqlx::query_as::<_,(u64,String)>(&sql_format!(
                    "select app_id,feature_key from {} where app_id in ({}) and feature_key  in ({}) and status={}",
                    AppFeatureModel::table_name(),
        ...
        Ok(out_data
            .into_iter()
            .map(|e| {
                let mut attr = AppAttrData {
                    exter_login: app_attr.check_inner_feature.as_ref().and_then(|e| {
                        if e.contains(&AppRequestType::ExterLogin) {
                            Some(false)
                        } else {
                            None
                        }
                    }),
                    oauth_client: app_attr.check_inner_feature.and_then(|e| {
                        if e.contains(&AppRequestType::OAuthClient) {
                            Some(false)
                        } else {
                            None
                        }
                    }),
                    sup_app: app_attr.check_inner_feature.and_then(|e| {
                        if e.contains(&AppRequestType::SubApp) {
                            Some(false)
                        } else {
                            None
                        }
                    }),
                    oauth_server: app_attr.check_inner_feature.and_then(|e| {
                        if e.contains(&AppRequestType::OAuthServer) {
                            Some(false)
                        } else {
                            None
                        }
                    }),
                    exter_feature: app_attr.check_exter_feature.map(|_| vec![]),
                    sub_app_count: if app_attr.sub_app_count {
                        None
                    } else {
                        Some(vec![])
                    },
                    oauth_client_data: None,
                    oauth_server_scope_data: None,
                    parent_app: None,
                };
                attr.sub_app_count = Some(
                    sub_count_data
                        .iter()
                        .filter(|t| t.0 == e.id)
                        .map(|t| (t.1, t.2))
                ...
                attr.exter_login = Some(
                    inn_feature_data
                        .iter()
                        .any(|t| t.0 == e.id && AppRequestType::ExterLogin == *t.1),
                );
                attr.oauth_client = Some(
                    inn_feature_data
                        .iter()
                        .any(|t| t.0 == e.id && AppRequestType::OAuthClient == *t.1),
                );
                attr.sup_app = Some(
                    inn_feature_data
                        .iter()
                        .any(|t| t.0 == e.id && AppRequestType::SubApp == *t.1),
                );
                attr.oauth_server = Some(
                    inn_feature_data
                        .iter()
                        .any(|t| t.0 == e.id && AppRequestType::OAuthServer == *t.1),
                );
                attr.exter_feature = app_attr.check_exter_feature.as_ref().map(|t| {
                    t.iter()
                        .map(|st| {
                            (
                                *st,
                                ext_feature_data
                                    .iter()
                                    .any(|mt| mt.0 == e.id && mt.1.as_str() == *st),
                            )
                        })
                        .collect::<Vec<_>>()
                });
                attr.oauth_server_scope_data = Some(
                ...
                attr.oauth_client_data = oauth_client_data
                    .iter()
                    .find(|t| t.app_id == e.id)
                    .map(|t| t.to_owned());
                attr.parent_app = parent_app_data
                    .iter()
...
impl App {
    ...
    //系统APP的数据
    pub async fn system_app_data<'t>(
        &self,
        app_where: &SystemAppParam<'t>,
        app_attr: Option<&AppAttrParam<'t>>,
...
impl App {
    ...
    //指定用户APP的数据
    pub async fn user_app_data<'t>(
        &self,
        user_id: u64,
        app_where: &UserAppDataParam<'t>,
        app_attr: Option<&AppAttrParam<'t>>,
        page: Option<&PageParam>,
    ) -> AppResult<Vec<(AppModel, AppAttrData<'t>)>> {
