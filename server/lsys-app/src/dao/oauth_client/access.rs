...
// oauth 登录服务器实现
pub struct AppOAuthClientAccess {
    auth: Arc<AccessAuth>,
    redis: deadpool_redis::Pool,
}

impl AppOAuthClientAccess {
...
impl AppOAuthClientAccess {
    /// 创建OAUTH CODE
    async fn save_code_param_valid(&self, code_data: &AccessOAuthCodeData<'_>) -> AppResult<()> {
    ...
    pub async fn save_code(
        ...
        self.save_code_param_valid(code_data).await?;
    ...
    /// 清理创建的 code
    pub async fn destroy_code(&self, app_id: u64, oauth_app_id: u64, code: &str) -> AppResult<()> {
    ...
    /// 根据code完成登录
    async fn code_do_login_param_valid(&self, code: &str) -> AppResult<()> {
        ValidParam::default()
            .add(
                valid_key!("oauth_code"),
                &code,
    ...
    /// 根据code完成登录
    pub async fn get_code(&self, app_id: u64, oauth_app_id: u64, code: &str) -> AppResult<String> {
        self.code_do_login_param_valid(code).await?;
        let mut redis = self.redis.get().await?;
        let save_key = create_save_key("code", app_id, oauth_app_id, code);
        let data_opt: Option<String> = redis.get(save_key.as_str()).await?;
        match data_opt {
            None => Err(AppError::System(fluent_message!("access-not-code"))),
            Some(data) => Ok(data),
        }
    }
