...
type ProviderBuilderFn = Box<
    dyn Fn(serde_json::Value) -> Pin<Box<dyn Future<Output = FileResult<Box<dyn OssProvider>>> + Send>>
        + Send
        + Sync,
>;
...
impl OssProviderRegistry {
    ...
    where
        C: crate::common::OssProviderConfig
            + serde::de::DeserializeOwned
            + Send
            + 'static,
        P: OssProvider,
    {
        self.add_builder(P::provider_type(), Box::new(|json| {
            Box::pin(async move {
