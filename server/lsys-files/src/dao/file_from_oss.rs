...
impl FileDao {
    ...
    pub async fn create_from_local_upload_oss(
        ...
        // 先检查文件是否已存在
        if let Some(existing) = self.helper.find_existing_file(storage_type, &file_md5).await? {
            info!(
            ...
            return self
                .handle_existing_file(existing, &temp_oss_result, user_id, add_user_id, app_id, tag_names)
                .await;
        }
        ...
        let oss_result = oss_provider.upload_from_local(local_path, &upload_info).await?;
        ...
        // 调用不存在处理函数完成入库
        self.create_new_file_record(oss_result, user_id, add_user_id, app_id, storage_type, tag_names, env_data)
            .await
    }
    ...
    async fn handle_existing_file(
        ...
        info!("handle_existing_file: existing file found, id={}", existing.id);
    ...
    pub async fn create_from_oss(
            ...
            .await?
        {
            ...
            // 文件已存在，调用处理存在文件的函数
            self.handle_existing_file(existing, &oss_result, user_id, add_user_id, app_id, tag_names)
                .await
        } else {
            // 文件不存在，调用创建新文件记录的函数
            self.create_new_file_record(oss_result, user_id, add_user_id, app_id, storage_type, tag_names, env_data)
    ...
    pub async fn sync_oss_to_local(
        ...
        if let Some(local) = self.helper.find_file_local_by_oss_file_id(file.id).await? {
            ...
            if let Some(local_file) = self.helper.find_file_by_id(local.file_id).await? {
                    ...
                    .await?
                {
                    ...
                    self.tag_dao
                        .batch_add_tags(local_file.id, file_user.user_id, file_user.app_id, &source_tag_refs, None)
                        .await?;
        ...
        let (_rel_path, full_path) = self.helper.create_new_file(&oss_prefix, oss_ext).await?;
        oss_provider
            ...
            .await?
        {
            Update::<_, FileLocalModel>::new()
                ...
                .execute(&self.helper.db, |qb| {
                    qb.push_where().field_eq("id", local_rec.id);
                })
                .await?;
        }
