...
use rxing::{
    BarcodeFormat, BinaryBitmap, BufferedImageLuminanceSource, EncodeHintType, EncodeHintValue,
    EncodingHintDictionary, MultiFormatReader, MultiFormatWriter, MultiUseMultiFormatReader,
    RXingResult, Reader, Writer,
};
...
impl BarCodeCore {
    pub async fn decode(
        ...
        if let Some(ref character_set) = param.character_set {
            hints.insert(
                rxing::DecodeHintType::CHARACTER_SET,
                rxing::DecodeHintValue::CharacterSet(character_set.to_owned()),
            );
        }
        if let Some(ref allowed_lengths) = param.allowed_lengths {
            hints.insert(
        ...
        if let Some(ref allowed_ean_extensions) = param.allowed_ean_extensions {
            hints.insert(
        ...
        if extension == "svg" {
            ...
            let image_format = ImageFormat::from_extension(extension).ok_or_else(|| {
                format!("format not support:{}",extension)
            })?;
            ...
            if param.decode_multi.unwrap_or(false) {
                ...
                Ok(scanner.decode_multiple_with_hints(
                ...
                let result = multi_format_reader.decode_with_hints(
                    &mut BinaryBitmap::new(HybridBinarizer::new(
                        BufferedImageLuminanceSource::new(img),
                    )),
                    &hints,
