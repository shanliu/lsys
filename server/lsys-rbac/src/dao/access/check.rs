...
impl AccessCheckRes<'_> {
    ...
    // 系统待验证资源
    pub fn system_empty_data<'s>(res_type: &'s str, ops: Vec<&'s str>) -> AccessCheckRes<'s> {
...
impl AccessCheckEnv<'_> {
    pub fn using_user_id(mut self, user_id: u64) -> Self {
        self.user_id = user_id;
        self
    }
    pub fn using_login_token(mut self, token: &str) -> Self {
...
impl RbacAccess {
    pub async fn check(
        ...
        let access_role_list = if !self.is_root(&env_data.user_id) {
            ...
            let res_check = &tmp_check
                        ...
                        .flat_map(|r| {
                            let op_data = r
                                ...
                                .filter_map(|o| {
                                    op_list
                                        .iter()
                                        .find(|r| r.0.op_key == *o && *tu_id == r.0.user_id)
                ...
                .collect::<Vec<_>>();
            let role_check = session_role_data
                .iter()
        ...
        for res_item in check_res_data {
            ...
            for op_item in res_item.op_key_data.iter() {
                let op_detail = op_list
                    .iter()
                    .find(|e| e.0.op_key == *op_item && e.0.user_id == res_item.user_id)
                    .and_then(|e| e.1.as_ref());
        ...
        let bad_item=check_data.iter().map(|check_item|{
            AccessUnauthRes{
                ...
                msg:if check_item.is_role_excluce {
                    fluent_message!( "rbac-access-block",{
                    ...
                    }else{ fluent_message!( "rbac-access-unauth",{
                        "res_name":check_item.res_detail.as_ref().map(|e|e.res_name.as_str()).unwrap_or(check_item.check_res_item.res_type),
                        "op_name":check_item.op_detail.as_ref().map(|e|e.op_name.as_str()).unwrap_or(check_item.op_key),
                        "user_id":check_item.check_res_item.user_id,
                    })},
    ...
    pub async fn list_check(
        ...
        for check_res_data in check_res_list {
            match self
                .check(env_data, session_role_data, check_res_data)
                .await
            {
                Ok(()) => return Ok(()),
                Err(err) => match err {
...
impl RbacAccess {
    async fn check_add_audit(
        ...
        let role_key_data = json!(session_role_data
            .iter()
            .map(|e| {
                json!({
                    "key":e.role_key,
