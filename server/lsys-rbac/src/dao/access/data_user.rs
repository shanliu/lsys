...
pub struct AccessPublicResUserData {
    ...
    pub exist_system_logged_all: bool, //role list 存在系统授权的登录用户可访问  -> 可通过 find_role_list_from_res 返回角色列表
    pub exist_system_all_all: bool, //role list 存在系统授权的任意用户可访问  -> 可通过 find_role_list_from_res 返回角色列表
    pub exist_system_user_all: bool, //user role list 存在系统授权的特定用户可访问 -> 可通过 find_user_list_from_res 返回用户列表
    pub exist_self_logged_all: bool, //role list  -> 可通过 find_role_list_from_res 返回角色列表
    pub exist_self_all_all: bool,    //role list  -> 可通过 find_role_list_from_res 返回角色列表
...
impl RbacAccess {
        ...
        for (db_user_id, db_user_range, db_res_range) in data {
            ...
            if db_user_id == 0 {
                if RbacRoleUserRange::Any.eq(db_user_range) {
                    pub_access.exist_system_all_all = true;
                }
                if RbacRoleUserRange::Logged.eq(db_user_range) {
                    pub_access.exist_system_logged_all = true;
                }
                ...
                if RbacRoleUserRange::Any.eq(db_user_range) {
                    pub_access.exist_self_all_all = true;
                }
                if RbacRoleUserRange::Logged.eq(db_user_range) {
                    pub_access.exist_self_logged_all = true;
                }
                if RbacRoleUserRange::Session.eq(db_user_range) {
    ...
    async fn user_data_pub_sql(&self, user_id: u64) -> [String; 2] {
        ...
        [sql_format!(
            ...
            from {} as role 
            where role.status ={} and role.user_id in ({})
            and role.res_range = {} and role.user_range in ({}) 
            ...
            &[RbacRoleUserRange::Any as i8,RbacRoleUserRange::Logged as i8,RbacRoleUserRange::Session as i8],
        ),sql_format!(
            "select role.user_id,role.user_range,role.res_range
...
pub struct AccessResUserData {
    //user_id=0
    pub exist_system_all_exclude: bool, //role list -> 可通过 find_role_list_from_res 返回角色列表
    pub exist_system_all_include: bool, //role list -> 可通过 find_role_list_from_res 返回角色列表
    pub exist_system_logged_exclude: bool, //role list -> 可通过 find_role_list_from_res 返回角色列表
    pub exist_system_logged_include: bool, //role list -> 可通过 find_role_list_from_res 返回角色列表
    ...
    //user_id>0
    pub exist_self_all_exclude: bool, //role list -> 可通过 find_role_list_from_res 返回角色列表
    pub exist_self_all_include: bool, //role list -> 可通过 find_role_list_from_res 返回角色列表
    pub exist_self_logged_exclude: bool, //role list -> 可通过 find_role_list_from_res 返回角色列表
    pub exist_self_logged_include: bool, //role list -> 可通过 find_role_list_from_res 返回角色列表
...
impl RbacAccess {
    ...
    pub async fn find_user_data_from_res(
        ...
        let mut res_access = AccessResUserData {
            exist_system_all_include: false,
            exist_system_logged_include: false,
            exist_system_session_include: false,
            exist_system_user_include: false,
            exist_self_all_include: false,
            exist_self_logged_include: false,
            exist_system_user_exclude: false,
            exist_self_user_include: false,
            exist_system_all_exclude: false,
            exist_system_logged_exclude: false,
            exist_system_session_exclude: false,
            exist_self_all_exclude: false,
            exist_self_logged_exclude: false,
        ...
        match self
            ...
            .await
        {
            Ok(res_row) => match self.op.find_one_by_info(&OpInfo { op_key, user_id }).await {
                Ok(op_row) => {
                    let sql = [sql_format!(
                            "select role.user_id,role.user_range,role.res_range
                            from {} as perm  
                            ...
                            and role.res_range in ({}) and role.user_range in ({}) 
                            group by role.user_id,role.user_range,role.res_range",
                    ...
                    for (db_user_id, db_user_range, db_res_range) in data {
                        if RbacRoleResRange::Exclude.eq(db_res_range) {
                            if db_user_id == 0 {
                                if RbacRoleUserRange::Any.eq(db_user_range) {
                                    res_access.exist_system_all_exclude = true;
                                }
                                if RbacRoleUserRange::Logged.eq(db_user_range) {
                                    res_access.exist_system_logged_exclude = true;
                                }
                                if RbacRoleUserRange::Session.eq(db_user_range) {
                                ...
                                if RbacRoleUserRange::Any.eq(db_user_range) {
                                    res_access.exist_self_all_exclude = true;
                                }
                                if RbacRoleUserRange::Logged.eq(db_user_range) {
                                    res_access.exist_self_logged_exclude = true;
                                }
                                if RbacRoleUserRange::Session.eq(db_user_range) {
                            ...
                            if db_user_id == 0 {
                                if RbacRoleUserRange::Any.eq(db_user_range) {
                                    res_access.exist_system_all_include = true;
                                }
                                if RbacRoleUserRange::Logged.eq(db_user_range) {
                                    res_access.exist_system_logged_include = true;
                                }
                                if RbacRoleUserRange::Session.eq(db_user_range) {
                                ...
                                if RbacRoleUserRange::Any.eq(db_user_range) {
                                    res_access.exist_self_all_include = true;
                                }
                                if RbacRoleUserRange::Logged.eq(db_user_range) {
                                    res_access.exist_self_logged_include = true;
                                }
                                if RbacRoleUserRange::Session.eq(db_user_range) {
...
impl RbacAccess {
    ...
    fn find_user_list_sql_from_res(
        &self,
        user_id: u64,   //资源用户ID
        res_type: &str, //资源类型
        res_data: &str, //资源数据
        ...
        if res_range_any {
            sql.push(sql_format!(
                ...
                from {} as role  
                join {} as role_user on role_user.role_id=role.id
                where  role.status ={} and role_user.status={} and role.user_id in ({})
                and role.res_range = {} and role.user_range = {} ",
                field,
                RbacRoleModel::table_name(),
                RbacRoleUserModel::table_name(),
        ...
        if res_range_exclude {
            sql.push(sql_format!(
                ...
                and role.user_id in ({}) and role.res_range = {} and role.user_range = {} 
                and op.op_key={op_key} and res.res_type={res_type} and res.res_data={res_data}",
                field,
                RbacRoleModel::table_name(),
                RbacRoleUserModel::table_name(),
                RbacPermModel::table_name(),
    ...
    //已配置的特定用户
    #[allow(clippy::too_many_arguments)]
    pub async fn find_user_list_from_res(
        &self,
        user_id: u64,   //资源用户ID
        res_type: &str, //资源类型
        res_data: &str, //资源数据
       ...
       "#;
        let sql = self.find_user_list_sql_from_res(
            user_id,  //资源用户ID
            res_type, //资源类型
            res_data, //资源数据
            op_key,   //授权操作结构列表,
    ...
    //已配置的特定用户
    #[allow(clippy::too_many_arguments)]
    pub async fn find_user_count_from_res(
        &self,
        user_id: u64,   //资源用户ID
        res_type: &str, //资源类型
        res_data: &str, //资源数据
...
impl RbacAccess {
    ...
    //除特定配置用户外的角色列表
    #[allow(clippy::too_many_arguments)]
    fn find_role_list_sql_from_res(
        &self,
        user_id: u64,   //资源用户ID
        res_type: &str, //资源类型
        res_data: &str, //资源数据
        ...
        let user_range = match user_range {
            Some(tmp) => {
                if tmp.is_empty() {
                    return Ok(vec![]);
                }
                tmp.iter().map(|e| *e as i8).collect()
            }
            None => vec![
        ...
        if res_range_exclude {
            sql.push(sql_format!(
               ...
               and role.user_id in ({}) and role.res_range = {} and role.user_range in ({}) 
               and op.op_key={op_key} and res.res_type={res_type} and res.res_data={res_data}",
                field,
                RbacRoleModel::table_name(),
                RbacPermModel::table_name(),
    ...
    //除特定配置用户外的角色列表
    #[allow(clippy::too_many_arguments)]
    pub async fn find_role_list_from_res(
        &self,
        user_id: u64,   //资源用户ID
        res_type: &str, //资源类型
        res_data: &str, //资源数据
       ...
       "#;
        let sql = self.find_role_list_sql_from_res(
            user_id,
            res_type,
            res_data,
            op_key,
    ...
    #[allow(clippy::too_many_arguments)]
    pub async fn find_role_count_from_res(
        &self,
        user_id: u64,   //资源用户ID
        res_type: &str, //资源类型
        res_data: &str, //资源数据
