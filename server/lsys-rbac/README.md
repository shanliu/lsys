...
1. 约定:当资源的用户id=0时，该用户为游客
2. 约定:当资源的用户id>0时，为指定用户的ID

#### 资源定义
> 需要被授权管理的对象,如:用户发布的文章,电商中的商品等

1. 约定:资源组成为 用户ID+`资源标识符`
...
1. 约定:对资源的每一种操作权限,定义一个`资源权限标识符`进行管理
2. 验证权限时:传入 `资源标识符` + 用户ID + `资源权限标识符` 来进行权限校验
3. 验证权限时:在调用时,决定指定 `资源权限标识符` 是否是可选的授权校验.(适应场景:当资源被建立时需要校验授权,未被建立时无需校验)

#### 角色定义
> 角色是一组权限的集合,可被分配给用户或者其他实体

1. 约定:当角色的用户id=0时，该角色为系统的内置角色
2. 约定:当角色的用户id>0时，该角色为用户维护及控制的角色
...
> 角色用户范围定义可选值
```
1. 任何用户(RbacRoleUserRange::Guest):角色属于任意用户,包含未登陆访客跟已登录的用户
2. 登录用户(RbacRoleUserRange::Login):角色属于已登录的用户
3. 指定用户(RbacRoleUserRange::User):角色属于指定的用户,用户ID跟角色关系由`rbac_role_user`表维护.
4. 指定关系(RbacRoleUserRange::Relation):此类角色仅在用户鉴权时,通过传入`指定关系角色标识符`参数获取及使用.(注意:此角色跟用户是否登录无关)
```

```mermaid
flowchart TB
    subgraph 角色包含的用户范围关系
    subgraph "全部用户"
    subgraph "登录用户"
        subgraph "指定用户"
        end
...
1. 当角色用户范围定义为 `指定关系(RbacRoleUserRange::Relation)`,可认为此角色为外部定义角色
...
2. 授权访问自身资源(RbacRoleUserRange::AllowSelf):该角色可访问,跟本角色相同用户ID的资源
3. 自定义配置访问资源(RbacRoleUserRange::AllowCustom):此角色可访问指定资源,可访问资源关系由`yaf_rbac_role_op`表维护.(RBAC模型)
4. 禁止访问任何资源(RbacRoleUserRange::DenyAll):该角色禁止访问任意资源
```
```mermaid
flowchart TB
    subgraph 角色可访问资源的范围
...
1. 约定: 当角色用户为0,且范围是`授权访问自身资源(RbacRoleUserRange::AllowSelf)`时: 该角色可访问 资源用户ID为0(系统资源) 的权限
2. 约定: 当角色用户ID>0,且范围是`授权访问自身资源(RbacRoleUserRange::AllowSelf)`时: 该角色可访问 资源用户ID 等于 角色用户ID(该用户资源) 的权限
...
1. 根据用户传入的用户属性,资源属性,环境属性.过滤出需要校验权限的资源列表.例如:当用户为`超级管理员`时,不需要校验任何资源权限
2. 根据已过滤出资源,在根据跟当前用户属性,环境属性,得到可被当前用户访问的权限列表

> 备注:系统`超级管理员`是通过 内置角色 `SystemRoleCheckData` 实现的,在配置文件app.toml[root_user_id] 中指定 管理员用户ID
...
> 此功能需由外部系统自行解决跟维护
```
例如:当进行文章删除时,必须要文章查看权限为前提.所以在校验权限时需传入：查看 + 删除 的资源标识符
```
...
```
1. 用户ID,游客传入时用户ID为0
2. 收集需要校验权限的 资源权限标识符 参见(问题1,问题2)[check_vec]
...
1. 简单控制资源是否可被访问，如指定页面是否可被访问

```
1. 通过接口或后台建立资源
2. 建立角色
    优先级:根据实际情况设置,要全局控制把优先级设置最高即可
    用户范围:全部用户(RbacRoleUserRange::Guest)
    授权范围:自定义配置访问资源(RbacRoleUserRange::AllowCustom):关联上面建立资源
```


2. 控制资源是否可被指定角色的用户访问[RBAC]
...
3. 控制资源是否可被特定角色访问[CBAC]

> 例如粉丝关系，当为粉丝关系时，权限校验传入特定角色参数

> 例如特定IP可访问，当IP为指定IP时，权限校验传入特定角色参数
...
2. 定义资源：{用户b}资源 操作: {文章id}查看
3. 定义关系key：firend-{b用户id} 用户b在用户后台或通过接口配置该关系的权限
4. 访问时，授权传入关系key:firend-{b用户id}
```
...
1. 建立角色
    优先级:设置为最高
    用户范围:任何用户(RbacRoleUserRange::Guest)
