...
> 授权方[系统或指定用户] 对 访问方[被授权用户或游客] 在进行 指定资源(系统资源或用户资源)访问时 进行授权的行为
...
> 角色+资源[user_id=0]为系统rbac模型，角色+资源[user_id>0]为指定用户的rbac模型

> 角色+资源[user_id=0]时，可往角色添加用户的资源,即 资源[user_id>0] 添加到 角色[user_id=0] 中
