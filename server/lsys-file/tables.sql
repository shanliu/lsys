...
CREATE TABLE `lst_file_local` (
    ...
    `public_file_id` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '关联的公开存储文件ID(storage_type=local_public)',
    `private_file_id` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '关联的私有存储文件ID(storage_type=local_private)',
    `crypto_file_id` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '关联的加密存储文件ID(storage_type=local_crypto)',

    `from_oss_file_id` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '从OSS同步新增文件,查日志用,无相关业务逻辑',
    ...
    KEY `public_file_id` (`public_file_id`) USING BTREE,
    KEY `private_file_id` (`private_file_id`) USING BTREE,
    KEY `crypto_file_id` (`crypto_file_id`) USING BTREE
...
CREATE TABLE `lst_file_oss` (
  ...
  `local_file_id` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '对应的 本地文件(lst_file_local.file_id) lst_file.id',
...
-- 迁移脚本：为 lst_file_local 表添加关联字段
-- ALTER TABLE `lst_file_local` ADD COLUMN `public_file_id` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '关联的公开存储文件ID(storage_type=local_public)' AFTER `file_chunk_size`;
-- ALTER TABLE `lst_file_local` ADD COLUMN `private_file_id` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '关联的私有存储文件ID(storage_type=local_private)' AFTER `public_file_id`;
-- ALTER TABLE `lst_file_local` ADD COLUMN `crypto_file_id` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '关联的加密存储文件ID(storage_type=local_crypto)' AFTER `private_file_id`;
-- ALTER TABLE `lst_file_local` ADD KEY `public_file_id` (`public_file_id`) USING BTREE;
-- ALTER TABLE `lst_file_local` ADD KEY `private_file_id` (`private_file_id`) USING BTREE;
-- ALTER TABLE `lst_file_local` ADD KEY `crypto_file_id` (`crypto_file_id`) USING BTREE;
