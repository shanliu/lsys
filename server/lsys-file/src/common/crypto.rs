...
impl DecryptIterator {
    ...
    pub async fn next_chunk(&mut self) -> Option<io::Result<(ChunkBlock, Vec<u8>)>> {
        ...
        // 异步读取并解密块数据
        let result = decrypt_chunk(&self.key, &self.file_path, &chunk).await;
...
pub async fn encrypt_file(
    ...
    let key: &[u8; 32] = key.try_into().map_err(|_| {
        io::Error::new(
            ...
            format!("AES-256 key must be 32 bytes, got {}", key.len()),
        )
    })?;
...
pub async fn decrypt_file_range(
    ...
    Ok(DecryptIterator::new(
        key,
        encrypted_path.to_path_buf(),
        file_size,
        offset,
        end_offset,
    ))
...
/// 异步解密单个块
async fn decrypt_chunk(key: &[u8; 32], file_path: &Path, chunk: &ChunkBlock) -> io::Result<Vec<u8>> {
    let mut file = File::open(file_path).await?;

    // 定位到块位置
    file.seek(tokio::io::SeekFrom::Start(chunk.file_offset))
        .await?;
...
/// * `Ok(u64)` - 原始文件大小
/// * `Err(io::Error)` - 获取失败
pub async fn get_decrypted_size(encrypted_path: impl AsRef<Path>) -> io::Result<u64> {
...
/// * `Ok(false)` - 文件无法解密
/// * `Err(io::Error)` - 验证过程出错
pub async fn verify_encrypted_file(key: &[u8], encrypted_path: impl AsRef<Path>) -> io::Result<bool> {
    let key: &[u8; 32] = key.try_into().map_err(|_| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            format!("AES-256 key must be 32 bytes, got {}", key.len()),
    ...
    match decrypt_chunk(key, encrypted_path, &chunk).await {
        Ok(_) => Ok(true),
        Err(_) => Ok(false),
    }
}
...
pub async fn decrypt_file_to(
    ...
    // 创建解密迭代器
    let mut iter = decrypt_file_range(key, encrypted_path, 0, None).await?;
...
mod tests {
    ...
    async fn test_encrypt_decrypt() {
        ...
        // 加密
        encrypt_file(b"",&source_path, &encrypted_path).await.unwrap();
        assert!(tokio::fs::try_exists(&encrypted_path).await.unwrap());

        // 解密全部内容
        let mut decrypted_data = Vec::new();
        let mut iter = decrypt_file_range(b"",&encrypted_path, 0, None).await.unwrap();
    ...
    async fn test_partial_decrypt() {
        ...
        let mut decrypted_data = Vec::new();
        let mut iter = decrypt_file_range(b"",&encrypted_path, 10, Some(10))
            .await
            .unwrap();
