...
impl FileTagDao {
    ...
    pub async fn batch_add_tags(
        ...
        let existing_tags: Vec<FileTagModel> = qb.build_query_as().fetch_all(&self.db).await?;

        let existing_names: std::collections::HashSet<&str> =
            existing_tags.iter().map(|t| t.tag_name.as_str()).collect();
        ...
        if !new_tags.is_empty() {
            ...
            let mut batch_insert = BatchInsert::<_, FileTagModel>::with_capacity(new_tags.len());
        ...
        let res = Update::<_, FileTagModel>::new()
            ...
            .execute(OptionTxExecutor::new(transaction, &self.db), |qb| {
                qb.push_where()
                    ...
                    .field_eq("app_id", app_id)
                    .push_and()
                    .field_eq("tag_name", tag_name.clone())
                    .push_and()
                    .field_eq("status", FileTagStatus::Normal as i8);
            })
            .await?;
