...
mail-send-error       = 邮件发送失败:{$msg}
mail-send-reply-check   = 邮箱回复地址错误:{$msg}
sms-send-check = 手机号错误:{$msg}
 
user-external-oauth-error = 加载外部账号登陆配置失败:{$msg}
user-external-login-url = 加载外部账号登陆url失败:{$msg}
 
user-external-call = 请求外部账号信息:{$msg}

user-external-other-bind = 账号{$name}已绑定其他用户:{$name}
rbac-edit-role-bad-user = 该角色不能添加其他用户资源
rbac-unkown-res    = 资源{$res}不存在
rbac-bad-res-op     = 资源操作id[{$op_id}]不可用
mail-use-other-user-app = 非法进行app发送邮箱操作
sms-use-other-user-app = 非法进行app发送短信操作
