   ...
   auth_dao: UserAuthQuery,
    ...
    web_dao: Data<WebDao>,
     ...
     if let Err(e) = auth_dao.set_request_token(&bearer).await {
        ...
        let rx = match file_download_progress_sse(
            &file_ref_ids,
            &req_query,
            &auth_dao,
            web_dao.as_ref(),
        ...
        let sse_stream = stream! {
            let mut rx = rx;
            tracing::info!("[sse] stream started, waiting for messages");
            loop {
                match timeout(Duration::from_secs(15), rx.recv()).await {
                    Ok(Some(info)) => {
                        tracing::info!("[sse] yielding file_id={} pct={:.1}", info.file_id, info.percent);
                    ...
                    Ok(None) => break,
                    Err(_) => {
                        // SSE comment heartbeat: keeps long-lived stream active through proxies/compressors.
