...
use lsys_web::handler::api::user::app_export_task::{
    ExportActiveCountParam, ExportDownloadParam, ExportListParam, ExportSubmitParam,
    app_export_active_count, app_export_download, app_export_list, app_export_submit,
    app_export_task_mapping,
};
...
pub async fn export_task(
    ...
    Ok(match path.into_inner().as_str() {
        "mapping" => app_export_task_mapping(&req_query).await,
        "export_active_count" => {
            app_export_active_count(&json_param.param::<ExportActiveCountParam>()?, &req_query, &auth_dao, web_dao.as_ref()).await
        }
        "export_submit" => {
            app_export_submit(&json_param.param::<ExportSubmitParam>()?, &req_query, &auth_dao, web_dao.as_ref()).await
        }
        "export_list" => app_export_list(&json_param.param::<ExportListParam>()?, &req_query, &auth_dao, web_dao.as_ref()).await,
...
pub async fn export_task_download(
    ...
    // 调用下载函数
    let response = app_export_download(param, offset, &req_query, &auth_dao, web_dao.as_ref())
        .await
        .map_err(|e| req_query.fluent_error_json_response(&e))?;
    ...
    let stream = async_stream::stream! {
        while let Some(result) = iter.next_chunk().await {
            match result {
                ...
                Err(e) => {
                    yield Err(std::io::Error::other(req_query.fluent_error_string(&e.into())));                    break;
