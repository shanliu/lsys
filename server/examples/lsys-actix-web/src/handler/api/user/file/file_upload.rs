...
use lsys_web::handler::api::user::file::check_avatar_image;
...
///
/// 表单字段：
/// - `id`: 上传任务的 file_ref_id（由 upload_create 返回）
...
pub async fn file_upload_data(
    ...
    let result: lsys_web::common::JsonResult<lsys_web::common::JsonResponse> = async {
        // 头像仅允许图片类型，基于 magic bytes 校验，拒绝扩展名欺骗
        check_avatar_image(&file_data)?;
