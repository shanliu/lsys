...
pub struct UserFileAccessPathParam {
    pub key: String,
...
/// 逻辑：
/// - 公开文件：重定向到文件 URL（302）
/// - 私有文件：验证用户权限后流式输出文件内容
///
/// 支持 HTTP Range 请求头进行断点续传
#[post("/read", name = "user_file_access")]
pub async fn user_file_access(
    auth_dao: UserAuthQuery,
    req_query: ReqQuery,
    form: web::Form<UserFileAccessPathParam>,
    req: HttpRequest,
    ...
    // 调用 lsys-web 的文件访问函数
    let result = match lsys_web::handler::api::user::file::user_file_access(
        &lsys_web::handler::api::user::file::UserFileAccessParam {
            key: param.key,
        },
        offset,
        &req_query,
    ...
    match result {
        lsys_web::handler::api::user::file::UserFileAccessData::Url(url) => {
            // 公开文件：重定向
            HttpResponse::Found()
                .insert_header(("Location", url.as_str()))
                .finish()
        }
        lsys_web::handler::api::user::file::UserFileAccessData::Stream {
            ref_model,
            file_model,
            stream,
        } => {
...
/// - 公开文件：重定向到文件 URL（302）
/// - 私有文件：返回本地化错误文本（403）
#[get("/share/{key}", name = "user_file_access_public")]
pub async fn user_file_access_public(
    ...
    match lsys_web::handler::api::user::file::user_file_share(&key, web_dao.as_ref()).await {
