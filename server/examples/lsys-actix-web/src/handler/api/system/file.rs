...
pub async fn file(
    ...
    Ok(match path.into_inner().as_str() {
        ...
        "export_submit" => {
            admin_export_submit(&json_param.param::<AdminExportSubmitParam>()?, &auth_dao).await
        }
        "export_list" => {
            admin_export_list(&json_param.param::<AdminExportListParam>()?, &auth_dao).await
        }
        "oss_config_list" => {
            admin_oss_config_list(&json_param.param::<AdminOssConfigListParam>()?, &auth_dao).await
        }
        "oss_config_detail" => {
            admin_oss_config_detail(&json_param.param::<AdminOssConfigDetailParam>()?, &auth_dao)
                .await
        }
        "oss_config_add" => {
            admin_oss_config_add(&json_param.param::<AdminOssConfigAddParam>()?, &auth_dao).await
        }
        "oss_config_edit" => {
            admin_oss_config_edit(&json_param.param::<AdminOssConfigEditParam>()?, &auth_dao).await
        }
        "oss_config_delete" => {
            admin_oss_config_delete(&json_param.param::<AdminOssConfigDeleteParam>()?, &auth_dao)
                .await
        }
