...
use lsys_core::db::{Insert, QueryBuilderExt, Update, utils::FetchField};
...
impl FileCollector {
    ...
    pub async fn submit_task(
    ...
    ) -> FileManagerResult<(u64, u64, String)> {
        // 查询脚本
        let script = self.find_script_by_id(script_id).await?.ok_or_else(|| {
            FileManagerError::Message(fluent_message!("collector-script-not-found"))
        })?;
        ...
        let request_id_max = fetch_field
            .string_max::<CollectorRecordModel>(&CollectorRecordModel::REQUEST_ID)
            .await
            .len_or(64);
        let exec_params_max = fetch_field
            .string_max::<CollectorRecordModel>(&CollectorRecordModel::EXEC_PARAMS)
            .await
            .len_or(65535); // TEXT 默认最大长度
        ...
        let tag_script_record = format!("script_record_{}", record_id);
        ...
        let cb_request_id = request_id.to_string();
