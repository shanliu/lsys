...
impl WebExportTask {
    ...
    async fn dispatch_pending(&self) -> WebResult<()> {
        ...
        loop {
            ...
            let affected = Update::<_, ExportTaskModel>::new()
                ...
                .execute(&self.db, |qb| {
                    ...
                    qb.push_and().field_eq("status", ExportTaskStatus::Pending as i8);
                })
                .await?
                .rows_affected();
    ...
    async fn execute_task(
        ...
        file_dao: &Arc<lsys_files::dao::FileDao>,
        exporters: &HashMap<String, Box<dyn super::exporter::Exporter>>,
        record: ExportTaskModel,
    ) {
        let task_id = record.id;

        // 1. 查找 exporter
        let exporter = match exporters.get(&record.export_type) {
            Some(exp) => exp,
            None => {
        ...
        let result = exporter.export(record, params).await;
        ...
        match result {
            ...
            Ok(file_path) => {
                // 4. 将文件 Move 到 lsys-files 存储，打 TAG
                let path_str = file_path.to_string_lossy().to_string();
                let tag_export = format!("export_{}", task_id);
                let tag_type = format!("export_type_{}", export_type);
                let tag_app = format!("app_{}", app_id);
                let tag_names: Vec<&str> = vec![&tag_export, &tag_type, &tag_app];
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        let err_msg = format!("failed to save file to lsys-files: {}", e);
                        error!("export_task: task id={} error: {}", task_id, err_msg);
                        Self::mark_failed(db, task_id, &err_msg).await;
                    }
                }
