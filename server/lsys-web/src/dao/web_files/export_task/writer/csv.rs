...
impl CsvWriter {
    ...
    pub fn new(record: &ExportTaskModel) -> Self {
        let path = std::env::temp_dir()
            .join(format!("export_{}_{}.csv", record.export_type, record.id));
        Self(path)
    }
    ...
    pub async fn header<H: CsvRecord>(self, header: H) -> Result<CsvRowWriter<H::Arity>, WebError> {
        let mut file = tokio::fs::File::create(&self.0).await?;
        file.write_all(b"\xEF\xBB\xBF").await?; // UTF-8 BOM
        file.write_all(to_line(&header.to_fields()).as_bytes()).await?;
        Ok(CsvRowWriter {
            file,
            path: self.0,
            _arity: PhantomData,
...
impl<A> CsvRowWriter<A> {
    ...
    pub async fn write_row<R: CsvRecord<Arity = A>>(&mut self, row: R) -> Result<(), WebError> {
        self.file.write_all(to_line(&row.to_fields()).as_bytes()).await?;
        Ok(())
    }
    ...
    /// flush 并返回文件路径（交由调用方存入 lsys-files）。
    pub async fn finish(mut self) -> Result<PathBuf, WebError> {
