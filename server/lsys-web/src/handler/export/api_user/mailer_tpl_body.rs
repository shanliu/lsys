...
impl WebExporter for MailerTplBodyExporter {
    async fn check(
        &self,
...
impl Exporter<crate::dao::WebError> for MailerTplBodyExporter {
    fn export<'a>(
        &'a self,
    ...
    > {
        Box::pin(async move {
            ...
            let total = self
                .tpl_dao
                .list_count(app_id, Some(SenderType::Mailer), id, tpl_id, tpl_id_like)
                .await? as u64;
            if total > 0 {
