// 系统管理端邮件消息列表导出
//
//   SystemMailerMessageListExporter — 系统所有邮件消息列表
...
pub const EXPORT_TYPE_SYSTEM_MAILER_MESSAGE_LIST: &str = "system_mailer_message_list";

/// 系统邮件消息列表权限检查器
pub struct SystemMailerMessageListExportCheck {
    pub web_rbac: Arc<crate::dao::WebRbac>,
}

#[async_trait::async_trait]
impl WebExporterCheck for SystemMailerMessageListExportCheck {
    async fn check(
        &self,
        ...
        self.web_rbac
            .check(check_env, &CheckAdminMailMgr {})
            .await?;
        Ok(())
    }
}
...
/// 系统邮件消息列表导出器
pub struct SystemMailerMessageListExporter {
    pub mailer_dao: Arc<MailSenderDao>,
}

impl Exporter<crate::dao::WebError> for SystemMailerMessageListExporter {
    fn export<'a>(
        &'a self,
