// 系统管理员文件列表导出
//
//   AdminFileListExporter — 管理员全量文件列表
//     CSV 列: id, file_name, file_md5, file_size, storage_type, status, content_type, user_id, add_time
...
pub const EXPORT_TYPE_SYSTEM_ADMIN_FILE_LIST: &str = "system_admin_file_list";

/// 系统管理员文件列表权限检查器
pub struct AdminFileListExportCheck {
    pub web_rbac: Arc<crate::dao::WebRbac>,
}

#[async_trait::async_trait]
impl WebExporterCheck for AdminFileListExportCheck {
    async fn check(
        &self,
        ...
        self.web_rbac
            .check(check_env, &CheckAdminFileManage {})
            .await?;
        Ok(())
    }
}
...
/// 系统管理员文件列表导出器
pub struct AdminFileListExporter {
    pub file_dao: Arc<FileDao>,
}

impl Exporter<crate::dao::WebError> for AdminFileListExporter {
    fn export<'a>(
        &'a self,
    ...
    > {
        Box::pin(async move {
            ...
            let filter = FileDataListParam {
                user_id,
                app_id: None,
                local_url: params["url"].as_str(),
                source_url: params["source_url"].as_str(),
