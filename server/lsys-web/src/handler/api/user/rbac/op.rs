use crate::common::{JsonResult, PageParam, UserAuthQueryDao};
use lsys_access::dao::AccessSession;
use lsys_rbac::dao::OpDataParam;
use lsys_rbac::model::RbacOpModel;
pub struct RbacOpAddParam {
    pub op_key: String,
    pub op_name: String,
    pub data: String,
}
pub async fn op_add(param: &RbacOpAddParam, req_dao: &UserAuthQueryDao) -> JsonResult<u64> {
    let req_auth = req_dao.user_session.read().await.get_session_data().await?;

    let id = req_dao
        .web_dao
        .web_rbac
        .rbac_dao
        .op
        .add_op(
            req_auth.user_id(),
            &param.op_key,
            &param.op_name,
            req_auth.user_id(),
            None,
            Some(&req_dao.req_env),
        )
        .await?;
    Ok(id)
}

pub struct RbacOpEditParam {
    pub op_id: u64,
    pub op_key: String,
    pub op_name: String,
    pub data: String,
}

pub async fn op_edit(param: &RbacOpEditParam, req_dao: &UserAuthQueryDao) -> JsonResult<()> {
    let req_auth = req_dao.user_session.read().await.get_session_data().await?;

    let op = req_dao
        .web_dao
        .web_rbac
        .rbac_dao
        .op
        .find_by_id(&param.op_id)
        .await?;
    req_dao
        .web_dao
        .web_rbac
        .rbac_dao
        .op
        .edit_op(
            &op,
            Some(&param.op_key),
            Some(&param.op_name),
            req_auth.user_id(),
            None,
            Some(&req_dao.req_env),
        )
        .await?;
    Ok(())
}
pub struct RbacOpDelData {
    pub op_id: u64,
}

pub async fn op_del(param: &RbacOpDelData, req_dao: &UserAuthQueryDao) -> JsonResult<()> {
    let req_auth = req_dao.user_session.read().await.get_session_data().await?;

    let op = req_dao
        .web_dao
        .web_rbac
        .rbac_dao
        .op
        .find_by_id(&param.op_id)
        .await?;
    req_dao
        .web_dao
        .web_rbac
        .rbac_dao
        .op
        .del_op(&op, req_auth.user_id(), None, Some(&req_dao.req_env))
        .await?;
    Ok(())
}

pub struct RbacOpDataParam {
    pub op_name: Option<String>,
    pub op_key: Option<String>,
    pub ids: Option<Vec<u64>>,
    pub page: Option<PageParam>,
    pub count_num: Option<bool>,
}

pub async fn op_data(
    param: &RbacOpDataParam,
    req_dao: &UserAuthQueryDao,
) -> JsonResult<(Vec<RbacOpModel>, Option<i64>)> {
    let req_auth = req_dao.user_session.read().await.get_session_data().await?;

    let res = req_dao
        .web_dao
        .web_rbac
        .rbac_dao
        .op
        .op_data(
            &OpDataParam {
                user_id: req_auth.user_id(),
                op_name: param.op_name.as_deref(),
                op_key: param.op_key.as_deref(),
                ids: param.ids.as_deref(),
            },
            param.page.as_ref().map(|e| e.into()).as_ref(),
        )
        .await?;
    let count = if param.count_num.unwrap_or(false) {
        Some(
            req_dao
                .web_dao
                .web_rbac
                .rbac_dao
                .op
                .op_count(&OpDataParam {
                    user_id: req_auth.user_id(),
                    op_name: param.op_name.as_deref(),
                    op_key: param.op_key.as_deref(),
                    ids: param.ids.as_deref(),
                })
                .await?,
        )
    } else {
        None
    };
    Ok((res, count))
}
