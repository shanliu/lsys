...
pub async fn file_list(
    ...
    if !need_full_tag && !data.is_empty() {
        let file_ids: Vec<u64> = data.iter().map(|item| item.item.id).collect();
        for &fid in &file_ids {
            let count = req_dao
                .web_dao
                .web_files
    ...
    for item in &data {
        ...
        if !need_full_tag {
            let tag_count = tag_count_map.get(&item.item.id).copied().unwrap_or(0);
            obj["tag_count"] = json!(tag_count);
            // 仅返回第一个标签用于列表预览
            if tag_count > 0 {
                let first_tag = first_tag_map.get(&item.item.id);
...
pub async fn file_tags(
    ...
    let auth_data = req_dao.user_session.read().await.get_session_data().await?;
    let file_user = resolve_file_user(param.file_user_id, req_dao).await?;
