...
pub struct UserFileAccessParam {
    pub key: String,
}
...
/// - 公开文件：返回 URL（无需认证）
/// - 私有文件：验证权限后返回流
pub async fn user_file_access<'a, F, Fut>(
    param: &UserFileAccessParam,
    offset: u64,
    req_dao: &RequestDao,
    private_auth: F,
    web_dao: &WebDao,
) -> JsonResult<UserFileAccessData>
where
    F: FnOnce() -> Fut,
