// 用户级采集脚本管理接口（app_id>0，用户通过应用管理）
//
// 权限校验模式参考 handler/api/user/file/mod.rs 中的 app_check_get:
//   - 校验用户对 app 的编辑/查看权限 (CheckUserAppEdit / CheckUserAppView)
//   - 校验 app 状态
//   - 校验 app 开通了 COLLECTOR 功能特性

use crate::common::{JsonData, JsonResponse, JsonResult, UserAuthQueryDao};
use crate::dao::access::{
    ...
    let total = if param.count_num.unwrap_or(false) {
        Some(
            req_dao
                ...
                .web_files
                .collector
                .count_script_files(&script, Some(param.app_id))
                .await?,
        )
    } else {
        None
...
pub async fn submit_task(
    ...
    let (record_id, task_id, script_name) = req_dao
        ...
        .web_files
        .collector
        .submit_task(param.script_id, user_id, app.id, &request_id, &params)
        .await?;
    ...
    let total = if param.count_num.unwrap_or(false) {
        Some(
            req_dao
                ...
                .web_files
                .collector
                .count_record_files(&record, Some(param.app_id))
                .await?,
        )
    } else {
        None
