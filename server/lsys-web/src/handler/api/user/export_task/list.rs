use crate::common::{JsonData, JsonPageData, JsonResponse, JsonResult, ToOffsetPageParam, UserAuthQueryDao};
    ...
    let total = if param.count_num.unwrap_or(false) {
        Some(
            req_dao
                ...
                .web_files
                .export_task
                .count_tasks(user_id, Some(0), export_type_ref, param.status)
                .await?,
        )
    } else {
        None
