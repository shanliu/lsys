...
pub async fn user_address_add(
    ...
    req_dao
        .web_dao
        .web_rbac
        .check(&req_dao.access_env().await?, &CheckUserAddressBase {}, None)
        .await?;
    let id = req_dao
        .web_dao
        .web_user
...
pub async fn user_address_delete(
...
pub async fn user_address_list_data(req_dao: &UserAuthQueryDao) -> JsonResult<JsonData> {
    let auth_data = req_dao.user_session.read().await.get_session_data().await?;
