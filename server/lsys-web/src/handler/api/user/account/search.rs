...
pub async fn search(param: &SearchParam, req_dao: &UserAuthQueryDao) -> JsonResult<JsonData> {
    let user = req_dao
        .web_dao
        .web_user
        .user_dao
...
#[derive(Debug, Deserialize)]
pub struct IdSearchParam {
    pub user_id: u64,
}

pub async fn id_search(param: &IdSearchParam, req_dao: &UserAuthQueryDao) -> JsonResult<JsonData> {
    let udat = req_dao
        .web_dao
        .web_user
        .user_dao
