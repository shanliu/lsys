...
pub struct SmsTaskSendTimeNotify {
...
impl TimeOutTaskExec for SmsTaskSendTimeNotify {
    async fn exec(
        &self,
        max_lock_time: usize,
...
#[async_trait::async_trait]
impl TimeOutTaskNextTime for SmsTaskSendTimeNotify {
    async fn next_time(&self, max_lock_time: usize) -> Result<Option<u64>, String> {
            ...
            .await
        {
            ...
            Err(err) => {
                error!("get sms last runtime fail fail:{:?}", err);
                None
            }
        };
        let ctime = last_time
...
#[async_trait::async_trait]
impl TimeOutTaskExecutor for SmsTaskSendTimeNotify {
    type Exec = Self;
    type NextTime = Self;
}
